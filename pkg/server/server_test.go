...
func TestAgentTokenAuthenticationErrorsToken(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			p := NewProxyServer("", []ProxyStrategy{ProxyStrategyDefault}, 1, nil, &AgentTokenAuthenticationOptions{
				Enabled:             true,
...
func TestAddRemoveFrontends(t *testing.T) {
	...
	p := NewProxyServer("", []ProxyStrategy{ProxyStrategyDefault}, 1, nil, nil)
...
func baseServerProxyTestWithoutBackend(t *testing.T, validate func(*agentmock.MockAgentService_ConnectServer)) {
	...
	proxyServer := NewProxyServer(uuid.New().String(), []ProxyStrategy{ProxyStrategyDefault}, 1, nil, &AgentTokenAuthenticationOptions{})
